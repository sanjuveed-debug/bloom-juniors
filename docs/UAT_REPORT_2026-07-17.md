...
> **Verification addendum (17 July 2026, evening).** The fix commit `d527863` addressed BJ-003 (verified: no PIN in any client response; server-side `/api/parent-pin` in use) but its sync rework contained a fatal regression: a stray line (`if (includePin) row.parent_pin = guardian.pin`) inside `saveCloudProgress` referenced out-of-scope variables, throwing `ReferenceError` on **every** cloud save — silently (retries swallow it), so production wrote nothing to the cloud from ~11:00 UTC until the hotfix. Root cause isolated by traffic analysis (merge-reads present, write requests absent, outbox accumulating) and confirmed by running `mergeProgress` with real payloads (it succeeds — the throw was the stray line alone). Hotfix `e3b300d` (one-line deletion) deployed to production `assets/index-CqOFgNTc.js` and re-verified end-to-end: two-device transfer now preserves treasures (1→1), friendship (2→2), companion identity, trophies, and the 2/2 daily path; the outbox from the broken window drained and restored the previously stranded treasure. Parent Zone now shows Sessions 2 / Adventures 2 / 1 treasure owned. **BJ-001 FIXED · BJ-002 FIXED · BJ-003 FIXED.** Remaining known issue: star counts (dashboard ⭐ and Parent Zone "Total Stars") stay 0 even in-session on the Tiny Stars path — stars are not being awarded/counted there at all (not a sync issue; tracked as the top "should fix" item). Minor fixes for dropped taps, celebration copy, and mood check-in frequency were shipped in `d527863` but not independently re-verified.
